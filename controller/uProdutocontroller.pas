...
type
  ...
  public
    ...
    function GetVlrVendaPerc(vlrInf, vlrComp : Double; perc : Boolean) : Double;
...
function TProdutoController.GetVlrVendaPerc(vlrInf, vlrComp: Double;
  perc: Boolean): Double;
begin
  if (perc) then
   begin
     Result := vlrComp + ((vlrComp * vlrInf) / 100);
   end
  else
   begin
     result := (((vlrInf * 1) / vlrComp) - 1) * 100;
   end;
end;
...
begin
  ...
  if (dtSource.DataSet.FieldByName('COD_BARRA').AsString = '') then
    raise Exception.Create('Informe um código de barra para o produto, antes de continuar!');
