...
procedure TfrmCadProdutos.editsExit(Sender: Tobject);
begin
  if (Sender = edtPercLucro) then
    dtmTabelas.qryProdutoVLR_VENDA.AsFloat := produtoCTO.GetVlrVendaPerc(dtmTabelas.qryProdutoPERC_LUCRO.AsFloat,
                            dtmTabelas.qryProdutoVLR_COMPRA.AsFloat, True)
  else
  if (Sender = edtVlrVenda) then
   dtmTabelas.qryProdutoPERC_LUCRO.AsFloat := produtoCTO.GetVlrVendaPerc(dtmTabelas.qryProdutoVLR_VENDA.AsFloat,
                            dtmTabelas.qryProdutoVLR_COMPRA.AsFloat, False);
